...
The likelihood function for $\boldsymbol{\theta}$ and $\boldsymbol{\phi}$ is
$$
\mathcal{L}(\boldsymbol{\theta}, \boldsymbol{\phi}) = \exp\left(\sum_{i=1}^n{{y_i\theta_i-b(\theta_i)}\over{a(\phi_i)}}
+\sum_{i=1}^nc(y_i,\phi_i)\right).
$$
So the likelihood function for $\boldsymbol{\beta}$ and $\boldsymbol{\phi}$ is ($\mu_i = \mathbb{E}(Y_i) = b'(\theta_i)$)
$$
