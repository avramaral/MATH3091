...
font_add(
  family = "LM Roman 10",
  regular = "/Users/avra1d24/Library/Fonts/lmroman10-regular.otf"
)
showtext_auto()
```
...
# Flipped
ggplot(data.frame(x = x, y = y)) + geom_point(aes(x = x, y = y)) + labs(x = "Variable 1", y = "Variable 2") + scale_y_continuous(limits = c(0, 8), breaks = seq(0, 8, 1)) + theme_bw() + theme(text = element_text(size = 16, family = "LM Roman 10")) 
```
...
\begin{align*}
\frac{d}{d\theta}b(\theta) = b'(\theta) &= \frac{\exp \theta}{1 + \exp \theta} \\
                   \mu   &=  \frac{\exp \theta}{1 + \exp \theta} \implies \theta = \log\left(\frac{\mu}{1 - \mu}\right) = \text{logit}(\mu),
\end{align*}
i.e., the canonical link function for this example is $g(\mu) = \text{logit}(\mu)$. 
<!--
This follows since $\theta = b^{'-1}(\mu)$ and, for the canonical link function, $g(\mu)\equiv b^{'-1}(\mu)$. -->
...
The choice of link function (e.g. between the `logit` or `probit`) will have a large impact on the estimated parameters $\boldsymbol{\beta}$ (`beetle` [is analysed in Lab 05]{.alert}).
...
\theta_i &= b^{' -1}(\mu_i), \cr
\mu_i &= g^{-1}(\eta_i), \cr
\eta_i&=\mathbf{x}_i^{\top}\boldsymbol{\beta}=\sum_{j=1}^p x_{ij} \beta_j, \quad i = 1, \cdots, n.
\end{align*}

## Fitting a GLM in `R` (from [Computer Lab 05]{.alert})
...
```{r, echo = TRUE, fig.width = 5, fig.height = 4, fig.align = "center"}
g1 <- numDeriv::grad(func = function(x) { bin_loglike(x, y, m) }, x = 0.6495)
h1 <- numDeriv::hessian(func = function(x) { bin_loglike(x, y, m) }, x = 0.6495)
c(g1, h1)
```
...
:::{.super-small-text}
$$
\left(\left[\boldsymbol{\eta}^{(m)}+\mathbf{z}^{(m)}\right]-\mathbf{X}\boldsymbol{\beta}^{(m + 1)}\right)^{\top}\mathbf{W}^{(m)}\left(\left[\boldsymbol{\eta}^{(m)}+\mathbf{z}^{(m)}\right]-\mathbf{X}\boldsymbol{\beta}^{(m + 1)}\right)=
\sum_{i=1}^n w_i^{(m)}\left(\left[\eta_i^{(m)}+z_i^{(m)}\right]-\mathbf{x}_i^{\top}\boldsymbol{\beta}^{(m + 1)}\right)^2,
$$
:::
where $w_1^{(m)},\cdots ,w_n^{(m)}$ are the weights
and $\boldsymbol{\eta}^{(m)}+\mathbf{z}^{(m)}$ is called the [*adjusted dependent variable*]{.alert}.
:::
:::
...
:::{.callout-note .fragment}
For a particular generalised linear model, [the scaled deviance function describes how discrepancies between the observed and fitted values are penalised.]{.alert} Essentially, the deviance is a natural measure of “lack of fit” that aligns with the maximum likelihood approach in each distribution family.
:::
